...
class TestProjectAndToDoViewSet(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data_project = {'name': 'Test_1', 'description': 'Test_description', 'repository': 'https://github.com/1/',
                             'users': [1]}
        self.data_project_1 = {'name': 'Test_1', 'description': 'Test_description',
                               'repository': 'https://github.com/1/'}
        self.data_todo = {'project': 1, 'text': 'Test_1', 'user': 1}
        self.data_todo_mixer = mixer.blend(ToDo)
        self.data_project_put = {'name': 'Test_2', 'description': 'Test_description_2',
                               'repository': 'https://github.com/2/', 'users': [1]}
    ...
    def test_put_project_guest(self):
        ...
        project = Project.objects.create(**self.data_project_1)
        project.users.add(self.admin)
        response = client.put(f'{self.url_project}{project.id}/', self.data_project_put)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_put_project_admin(self):
        ...
        self.assertEqual(project_.description, self.data_project_put['description'])
        self.assertEqual(project_.repository, self.data_project_put['repository'])
        client.logout()
...
class TestProjectAndToDo(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.data_project = {'name': 'Test_1', 'description': 'Test_description',
                               'repository': 'https://github.com/1/'}
        self.data_todo = {'project': 1, 'text': 'Test_1', 'user': 1}
    ...
    def test_put_project_admin(self):
        ...
        response = self.client.put(f'{self.url_project}{project.id}/', self.data_project_put)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_put_todo_admin(self):
        ...
        project.users.add(self.admin)
        todo = ToDo.objects.create(project=project, text='Test_1', user=self.admin)
        self.client.login(username=self.name, password=self.password)
        response = self.client.put(f'{self.url_todo}{todo.id}/', self.data_todo_put)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
